...
%                             static image for for bg subtraction.
    ...
    imArray = nansen.module.twophoton.autosegmentation.flufinder.preprocess.removeBackground(imArray, opts);

    % Preprocess (subtract static background)
    optsNames = {'PrctileForBaseline'};
    opts = utility.struct.substruct(params, optsNames);
    %imArray = nansen.module.twophoton.autosegmentation.flufinder.preprocess.removeStaticBackground(imArray, opts);
    % Todo: Bin size should be part of options...
    imArray = nansen.module.twophoton.autosegmentation.flufinder.preprocess.removeDynamicBackground(imArray);
end
