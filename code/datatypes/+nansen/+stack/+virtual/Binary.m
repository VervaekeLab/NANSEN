...
% Binary - Class for to represent a virtual data adapter for a binary file.
    ...
    properties (Constant, Hidden)
        ...
        FILE_FORMATS = {'RAW', 'BIN', 'DAT'}   % Supported file formats
    end
    ...
    methods (Access = private)
        ...
        function removeIniFile(obj, iniFilePath)
        % removeIniFile - Remove ini file if it is created by this class
            
            % Todo: Consider adding a header/tag in the inifile created by
            % this class (writeinifile) to uniquely identify these files.
            try
                S = obj.readinifile(iniFilePath);
                
                % Assume the ini file is created by this class if it has the
                % fields "Class" and "Size"
                if isfield(S, 'Class') && isfield(S, 'Size')
                    delete(iniPath)
                end
            end
        end
