...
%   verifying schema/manifest consistency.
    ...
    methods (Test)
        ...
        function failedSetupStatusOverridesPathDetection(testCase)
        %failedSetupStatusOverridesPathDetection Failed setup remains retryable.
            import matlab.unittest.fixtures.TemporaryFolderFixture
            temporaryFolderFixture = testCase.applyFixture( ...
            ...
            entry = struct( ...
                ...
                'DependencyType', "community-toolbox", ...
                'InstallCheck', "nansen", ...
                'SetupHook', "nansen.module.twophoton.integration.flowreg.install");
            trackedAddon = struct( ...
                'Name', 'Flow Registration', ...
                'InstallationType', 'folder', ...
                'FilePath', temporaryFolderFixture.Folder, ...
                'ToolboxIdentifier', '', ...
                'IsInstalled', false, ...
                'SetupStatus', 'failed');

            result = nansen.internal.dependencies.checkInstallationStatus( ...
                entry, trackedAddon);

            testCase.verifyTrue(result.IsOnPath, ...
                'InstallCheck should still detect that code is on path')
            testCase.verifyFalse(result.IsInstalled, ...
                'Failed setup status should force a retryable install state')

            trackedAddon = rmfield(trackedAddon, 'SetupStatus');
            result = nansen.internal.dependencies.checkInstallationStatus( ...
                entry, trackedAddon);
            testCase.verifyFalse(result.IsInstalled, ...
                'Tracked add-ons with unknown setup status should retry setup')
        end
    end
