...
%   stub host whose empty DataLocationModel makes initialize() abort with a
...
%   initialize run" is observable without needing a project.
    ...
    methods (Test)
        ...
        %dataPropertyIsVisible Data is not Hidden, so it appears in display.
            mc = ?nansen.unittest.session.helper.HasSessionDataStub;
        ...
        %dataAccessReturnsSameSessionData obj.Data returns a stable SessionData.
            owner = nansen.unittest.session.helper.HasSessionDataStub();
            firstData = owner.Data;
            secondData = owner.Data;
        ...
        %displayShowsDataWithoutInitializing Data is shown but not initialized.
            shown = evalc('disp(nansen.unittest.session.helper.HasSessionDataStub())');
        ...
        %displayKeepsDynamicProperties Dynamic metadata props still appear.
            shown = evalc([ ...
                'e = nansen.unittest.session.helper.HasSessionDataStub(); ' ...
        ...
        %accessingDataTriggersInitialize First .Data access runs initialize().
            shown = evalc( ...
                'e = nansen.unittest.session.helper.HasSessionDataStub(); e.Data;');
