...
%   using isolated test manifests.
    ...
    methods (Test)
        ...
        function installationReportShowsFriendlyMexFailure(testCase)
        %installationReportShowsFriendlyMexFailure Summary avoids raw tracebacks.
            manager = testCase.Fixture.AddonManager;
            setupFailedResult = struct( ...
                'Name', 'Flow Registration', ...
                'IsRequired', false, ...
                'Status', 'failed', ...
                'Stage', 'setup', ...
                'Message', ['Flow Registration was downloaded, but its ', ...
                'compiled MEX components could not be built.'], ...
                'ErrorIdentifier', 'MATLAB:mex:BuildFailed', ...
                'ErrorMessage', 'Error using mex linker command failed', ...
                'LogFilePath', fullfile(tempdir, 'flow_registration.log'), ...
                'FilePath', '');
            downloadFailedResult = setupFailedResult;
            downloadFailedResult.Name = 'Example Download';
            downloadFailedResult.Stage = 'download';
            downloadFailedResult.Message = ...
                'Example Download could not be downloaded or installed.';
            downloadFailedResult.ErrorIdentifier = 'MATLAB:webservices:HTTP404StatusCodeError';
            downloadFailedResult.ErrorMessage = 'Not found';
            downloadFailedResult.LogFilePath = fullfile(tempdir, 'download.log');
            failedResults = [downloadFailedResult, setupFailedResult];
            installationReport = struct( ...
                'InstallationFolder', char(manager.InstallationFolder), ...
            ...
            testCase.verifyTrue(contains(outputText, ...
                'Some optional add-ons could not be fully set up.'))
            testCase.verifyTrue(contains(outputText, 'Download problems:'))
            testCase.verifyTrue(contains(outputText, 'Setup problems:'))
            testCase.verifyTrue(contains(outputText, ...
                'Flow Registration was downloaded'))
            testCase.verifyTrue(contains(outputText, 'Reason: Not found'))
            testCase.verifyTrue(contains(outputText, ...
                'Reason: Native MEX build failure.'))
            testCase.verifyTrue(contains(outputText, 'Error details:'))
            testCase.verifyFalse(contains(outputText, 'Full technical log:'))
            testCase.verifyTrue(contains(outputText, 'matlab:open('''))
            testCase.verifyTrue(contains(outputText, 'Open error details'))
            testCase.verifyFalse(contains(outputText, ...
                fullfile(tempdir, 'flow_registration.log') + "</a>"))
            testCase.verifyFalse(contains(outputText, ...
                'This looks like a native MEX build failure.'))
            testCase.verifyTrue(contains(outputText, 'matlab:web('''))
            testCase.verifyTrue(contains(outputText, 'Troubleshooting guide'))
            testCase.verifyTrue(contains(outputText, ...
                char(nansen.config.addons.InstallationReporter.TroubleshootingUrl)))
            testCase.verifyFalse(contains(outputText, ...
                char(nansen.config.addons.InstallationReporter.TroubleshootingUrl) + "</a>"))
            testCase.verifyFalse(contains(outputText, 'mex -setup C++'))
        end

        function installationErrorReportIsPlainText(testCase)
        %installationErrorReportIsPlainText Saved details avoid HTML links.
            try
                error('NANSEN:Test:InstallFailed', ...
            ...
            testCase.verifyTrue(contains(reportText, ...
                'Expected repository to contain a Readme and LICENSE file'))
            testCase.verifyFalse(contains(reportText, '<a href='))
            testCase.verifyFalse(contains(reportText, 'matlab:'))
        end
