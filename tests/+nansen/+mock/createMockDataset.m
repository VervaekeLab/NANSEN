function createMockDataset(datasetDirectory, options)
...
%   and metadata.
%
%   CREATEMOCKDATASET(__, "Verbose", true) prints progress messages to the
%   command window. Output is silent by default.
%
...
%   This function is used for testing the NANSEN toolbox.
    ...
    createDatasetDescription(datasetDirectory, verbose);
    ...
    for i = 1:length(subjects)
        ...
        createSubject(subjectDir, subjects{i}, verbose);
        ...
        for j = 1:length(sessions)
            ...
            createSession(sessionDir, subjects{i}, sessions{j}, brainRegion, imagingDepth, verbose);
        end
    end

    logStatus(verbose, 'Mock dataset created successfully in %s\n', datasetDirectory);
end

function createDatasetDescription(datasetDirectory, verbose)
% Create a dataset description file in JSON format
    ...
    logStatus(verbose, 'Created dataset description file\n');
end

function createSubject(subjectDir, subjectId, verbose)
% Create a subject directory with metadata
    ...
    logStatus(verbose, 'Created subject directory: %s\n', subjectId);
end

function createSession(sessionDir, subjectId, sessionId, brainRegion, imagingDepth, verbose)
% Create a session directory with metadata and neural data
    ...
    logStatus(verbose, 'Created session directory: %s/%s\n', subjectId, sessionId);
end
